<!-- Modal: Bild bearbeiten (Overlay über der Galerie, Museums-Anmutung) -->
<div id="editModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
    <div class="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-sm bg-gradient-to-br from-frame to-[#cdc2a4] p-2 shadow-frame ring-1 ring-black/50">
        <div class="rounded-sm bg-panel p-6 ring-2 ring-gold/30">
            ...
            <div class="mb-5 flex items-center justify-between">
                <h2 class="font-serif text-2xl font-semibold text-frame">Bild bearbeiten</h2>
                <button type="button" onclick="closeEditModal()" aria-label="Schliessen"
            ...
            <form id="edit_form" action="bild_bearbeiten" method="POST" enctype="multipart/form-data"
                ...
                <div>
                    <div class="rounded-sm bg-gradient-to-br from-frame to-[#cdc2a4] p-2 shadow-lg ring-1 ring-black/40">
                        <div class="bg-mat p-3 ring-2 ring-gold/30">
                            <div class="relative overflow-hidden bg-black">
                                <img id="edit_preview" src="" alt="Vorschau" class="aspect-[4/3] w-full object-cover">
                                <div id="edit_noimage" class="absolute inset-0 hidden flex-col items-center justify-center gap-2 bg-wall text-neutral-500">
                    ...
                    <!-- Bild-Steuerung -->
                    <input type="file" id="edit_file" name="filename" accept="image/*" class="hidden">
                    <input type="hidden" id="edit_keep" name="keepImage" value="1">
                    <div class="mt-3 flex flex-wrap gap-2">
                        <button type="button" onclick="document.getElementById('edit_file').click()"
                        ...
                        <button type="button" id="edit_remove" onclick="removeEditImage()"
                                class="inline-flex items-center gap-1.5 rounded-lg border border-red-500/50 bg-red-950/50 px-3 py-2 text-xs font-semibold text-red-200 transition hover:bg-red-900/60">
                    ...
                    <p id="edit_imgError" class="mt-2 hidden text-xs text-red-300">Bitte ein Bild auswählen</p>
                </div>

                <!-- Felder -->
                <div class="space-y-4">
                    <div>
                        <label for="edit_titel" class="mb-1 block text-sm font-medium text-neutral-300">Titel</label>
                        <input type="text" id="edit_titel" name="titel" placeholder="Titel eingeben"
                    ...
                    <div>
                        <label for="edit_beschreibung" class="mb-1 block text-sm font-medium text-neutral-300">Beschreibung</label>
                        <textarea id="edit_beschreibung" name="beschreibung" rows="4" placeholder="Beschreibung eingeben"
                    ...
                    <div>
                        <label for="edit_datum" class="mb-1 block text-sm font-medium text-neutral-300">Datum</label>
                        <input type="date" id="edit_datum" name="datum"
                    ...
                    <div>
                        <label for="edit_ort" class="mb-1 block text-sm font-medium text-neutral-300">Ort</label>
                        <input type="text" id="edit_ort" name="ort" placeholder="Ort eingeben" autocomplete="off"
                               class="w-full rounded-lg border border-line bg-wall px-3 py-2 text-neutral-100 placeholder-neutral-500 focus:border-gold focus:outline-none focus:ring-2 focus:ring-gold/40">
                    </div>
                    <label class="flex items-center gap-3 text-sm font-medium text-neutral-300">
                        <input type="checkbox" id="edit_oeffentlich" name="oeffentlich" value="Yes"
                    ...
                    <div class="flex justify-end gap-2 pt-2">
                        ...
                        <button type="submit" id="edit_submit"
